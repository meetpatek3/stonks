...
/**
 * Build the two balanced legs for a simple same-currency move: money leaves
 * `fromAccountId` (−) and arrives at `toAccountId` (+). Optionally attaches
 * quantity/security to the investment-side posting — callers choose which leg
 * receives it via `quantityOn`.
 */
export function balancedMovePostings(input: {
  amountMinor: bigint;
  ...
  if (input.quantity !== undefined || input.securityId !== undefined) {
    const target = input.quantityOn === "from" ? from : to;
    if (input.quantity !== undefined) target.quantity = input.quantity;
    if (input.securityId !== undefined) target.securityId = input.securityId;
  }
...
/**
 * Pick default From/To accounts for a journal type. Prefers the most recently
 * used non-EXTERNAL account as the household side of a deposit/withdrawal.
 */
export function defaultEntryAccounts(input: {
  type: JournalType;
  accounts: readonly EntryAccountOption[];
  mruId: string | null;
}): { fromId: string | null; toId: string | null } {
  const external = input.accounts.find((a) => a.type === "EXTERNAL");
  const cash = input.accounts.find((a) => a.type === "CASH");
  const investment = input.accounts.find((a) => a.type === "INVESTMENT");
  const nonExternal = input.accounts.filter((a) => a.type !== "EXTERNAL");
  const mru =
    (input.mruId && nonExternal.find((a) => a.id === input.mruId)?.id) ||
    cash?.id ||
    nonExternal[0]?.id ||
    null;
  ...
  switch (input.type) {
    ...
    case "INTEREST_EARNED":
      return { fromId: external?.id ?? null, toId: mru };
    case "WITHDRAWAL":
    case "FEE":
    case "INTEREST_CHARGED":
      return { fromId: mru, toId: external?.id ?? null };
    case "BUY":
      return {
        fromId: cash?.id ?? mru,
        toId: investment?.id ?? nonExternal.find((a) => a.id !== mru)?.id ?? null,
      };
    case "SELL":
      return {
        fromId: investment?.id ?? mru,
        toId: cash?.id ?? nonExternal.find((a) => a.id !== mru)?.id ?? null,
      };
    default:
      return {
        fromId: mru,
        toId:
          nonExternal.find((a) => a.id !== mru)?.id ?? external?.id ?? null,
      };
  }
}
